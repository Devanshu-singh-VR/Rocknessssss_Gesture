...
#left = tf.keras.models.load_model('Left_eye/')
#right = tf.keras.models.load_model('Right_eye/')
#mouth = tf.keras.models.load_model('mouth/')
...
while True:
    ...
    L_eye = cv2.resize(L_eye, (30, 30)).reshape(1, 30, 30, 1)
    R_eye = cv2.resize(R_eye, (30, 30)).reshape(1, 30, 30, 1)
    Mouth = cv2.resize(Mouth, (30, 30)).reshape(1, 30, 30, 1)

    #l_val = left.predict(L_eye)[0]
    #r_val = right.predict(R_eye)[0]
    #m_val = mouth.predict(Mouth)[0]
